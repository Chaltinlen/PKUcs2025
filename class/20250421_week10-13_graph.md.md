...
> Explanation:
>
> 6. First, create a graph in a function.
> 7. Intialize a starting node and destination node.
> 8. Create a list for the visited nodes and stack for the next node to be visited.
> 9. Call the graph function.
> 10. Initially, the stack is empty.Push the starting node into the stack (stack.append(start) ).
> 11. Mark the starting node as visited (visited.append(start) ).
> 12. Repeat this process until all the neighbours are visited in the stack till the destination node is found.
> 13. If the destination node is found exit the while loop.
> 14. If the destination node is not present then “Not found” is printed.
> 15. Finally, print the path from starting node to the destination node.
>
